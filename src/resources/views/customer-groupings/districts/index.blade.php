...
@section('content')
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
		...
		<div class="btn-toolbar mb-2 mb-md-0">
			<a href="{{ route('districts.create') }}" class="btn btn-primary" role="button"><span data-feather="plus"></span> Create</a>
		</div>
	</div>
		...
		<table class="table table-striped table-sm">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<a href="{{ route('districts.edit', ['district' => $district->id]) }}" class="btn btn-primary" role="button"><span data-feather="edit"></span> Edit</a>
						<form method="POST" action="{{ route('districts.destroy', ['district' => $district->id]) }}" style="display:inline;">
							@csrf
							@method('DELETE')
							<button class="btn btn-danger" type="submit" onclick="return confirm('Are you sure you want to delete {{ $district->name }}?');"><span data-feather="trash-2"></span> Delete</button>
						</form>
					</td>
				</tr>
				@empty
